...
const AdminSignin = () => {
  ...
  return (
    <div className="h-screen  flex items-center justify-center bg-gradient-to-b from-purple-300 to-white" >
        <div className='border flex flex-col gap-6 p-10 w-[600px] rounded-lg'>
            <h1 className='text-3xl font-bold text-center '> Admin Signin</h1>
            ...
            <button onClick={serverCalls} className='w-full h-14 text-white text-xl font-semibold transition-all ease-linear duration-500 rounded-md bg-green-700 hover:bg-green-500'>Login</button>
        </div>
    </div>
  )
}
...
function Labeled({title,type,placeholder, onChange}){
    ...
    return<>
    <label  htmlFor={title}>
        <h2 className='text-2xl mb-4 font-semi-bold text-black font-bold'>{title}</h2>
